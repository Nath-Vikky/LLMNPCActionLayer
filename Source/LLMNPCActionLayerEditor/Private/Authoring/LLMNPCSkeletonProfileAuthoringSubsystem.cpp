...
)
{
	...
	for (int32 Index = 0; Index < UE_ARRAY_COUNT(FingerNames); ++Index)
	{
		...
		if (!RightBone.IsNone())
		{
			...
			OpenPose.SemanticBoneRotations.Add(RightSemantic, DefaultBindings.RightFingerOpenRotations[Index]);
			PointPose.SemanticBoneRotations.Add(RightSemantic, DefaultBindings.RightFingerPointRotations[Index]);
		}
		if (!LeftBone.IsNone())
		{
			Profile.SemanticBoneMap.Add(LeftSemantic, LeftBone);
			OpenPose.SemanticBoneRotations.Add(LeftSemantic, DefaultBindings.LeftFingerOpenRotations[Index]);
			PointPose.SemanticBoneRotations.Add(LeftSemantic, DefaultBindings.LeftFingerPointRotations[Index]);
		}
	}
	Profile.FingerPoses = {OpenPose, PointPose};
